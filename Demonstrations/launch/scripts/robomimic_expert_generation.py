...
def get_env_config(raw_directory):
    ...
    for key in env_kwargs:
        ...
        if key == "controller_configs":
            config[key] = env_kwargs[key]
            config[key]["damping_ratio"] = env_kwargs[key]["damping"]
            config[key]["damping_ratio_limits"] = env_kwargs[key]["damping_limits"]
            del config[key]["damping"]
            del config[key]["damping_limits"]
        config[key] = env_kwargs[key]
...
def save_as_hdf5(args, env_args, ep, f, new_demo_name, traj, is_robosuite):
    ...
    grp_demo.attrs["env"] = args.config["env_name"]
