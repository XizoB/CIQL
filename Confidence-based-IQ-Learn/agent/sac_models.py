...
class DiagGaussianActor(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, hidden_dim, hidden_depth, log_std_bounds):
        ...
        self.trunk = utils.mlp(obs_dim, hidden_dim, 2 * action_dim, hidden_depth)  # 构建多层感知机MLP结构，输出2*action_dim代表均值和方差
    ...
    def forward(self, obs):
        ...
        # constrain log_std inside [log_std_min, log_std_max] 内部约束边界
...
class SingelDetachedCritic(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim, hidden_depth, args):
        super(SingelDetachedCritic, self).__init__()
        self.obs_dim = obs_dim
        self.action_dim = action_dim
        ...
        # 网络各层结构的权重与偏差参数
        self.Q_weights, self.Q_biases = utils.build_param_list(obs_dim + action_dim, hidden_dim, 1, hidden_depth)

    def Q(self, obs_action):
        # 每一层添加激活函数
        hidden_activation = nn.ReLU(inplace=True)
        x = obs_action
        hidden_depth = self.hidden_depth # 隐藏层数目，为了最后一层的时候不添加激活函数
        for weight, bias in zip(self.Q_weights, self.Q_biases):
            if hidden_depth == 0: # 最后一层不添加激活函数
                x = x.mm(weight.transpose(0, 1)) + bias
                break
            x = hidden_activation(x.mm(weight.transpose(0, 1)) + bias)
            hidden_depth -= 1
        return x
    ...
    def set_parameters(self, vector: torch.Tensor):
        pointer = 0
        for layer in range(len(self.Q_weights)):
            n_param = int(self.Q_weights[layer].shape[0] * self.Q_weights[layer].shape[1])
            self.Q_weights[layer] = vector[pointer: pointer + n_param].view(self.Q_weights[layer].shape)
            pointer += n_param
            n_param = self.Q_biases[layer].shape[0]
            self.Q_biases[layer] = vector[pointer: pointer + n_param].view(self.Q_biases[layer].shape)
            pointer += n_param
