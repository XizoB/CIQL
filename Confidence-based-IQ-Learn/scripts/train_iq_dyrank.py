...
from utils.utils import eval_mode, evaluate, soft_update, save, save_config, save_trajs, select_state, save_best
...
def cdynamic(args, expert_buffer, lamb, device):
    """
    ...
    expert_batch, indexes = expert_buffer.get_samples(batch_size, True)  # 把 conf 传入 Memory 类中，以在里面全局使用
...
def main(cfg: DictConfig):
    ...
    if args.C_aware.conf_learn=="IQ" or args.C_aware.conf_learn=="sail":
        ...
        conf = cdynamic(args, expert_memory_replay, args.C_aware.lamb, device)
        conf = torch.Tensor(conf)
        conf_num = deque(conf).count(1.5)
        reach_conf = (conf.sum()-conf_num*1.5)/(len(conf)-conf_num)
        print("最优先验概率:", conf.mean())
        print("靠近阶段最优先验概率:", reach_conf)
        ...
        if args.C_aware.conf_learn=="conf_expert":
            if args.C_aware.lamb >= 1:
                args.C_aware.lamb = args.C_aware.lamb
            else:
    ...
    info_label = expert_memory_replay.sample_all_expert_traj()
    save_trajs(info_label, reach_conf, args.C_aware.conf_learn, args.C_aware.lamb, args.train.boundary_angle, output_dir='trajs')
