...
from utils.utils import evaluate, eval_mode, save, save_best
...
def irl_reward(GAMMA, agent, state, action, next_state):
    ...
    irl_reward = torch.tanh(irl_reward.squeeze())
...
def main(cfg: DictConfig):
    ...
    if args.pretrain:
        ...
        irl_agent.load(pretrain_path, f'_{args.env.name}')

    # 初始化 Replay_Buffer
    memory_replay = Memory(REPLAY_MEMORY, device, args.seed)
    ...
    for epoch in count():                                   # 创建无限迭代, 开始训练智能体
        ...
        for episode_step in range(HORIZON):
            ...
            reward =  irl_reward(GAMMA, irl_agent, state, action, next_state)
