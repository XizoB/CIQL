...
def main(cfg: DictConfig):
    ...
    REPLAY_MEMORY = None
    HORIZON_THRESHOLD = args.eval.threshold
    MAX_EPS = args.expert.demos
    HORIZON = int(args.env.horizon)

    memory_replay = Memory(REPLAY_MEMORY, args.device, args.seed)
    total_steps = 0
    saved_eps = 0
    ...
    for epoch in count():
        ...
        episode_infos = None
        for time_steps in range(HORIZON):
        ...
        if len(traj) <= HORIZON_THRESHOLD:
            ...
            print('Ep {}\t Horizon: {}\tSaving Episode reward: {:.2f}\t'.format(epoch, len(traj), episode_reward))
        else:
