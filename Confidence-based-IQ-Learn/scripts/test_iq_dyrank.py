...
def main(cfg: DictConfig):
    ...
    for i in range(seed_num):
        ...
        succeed = []
        for epoch in range(args.eval.eps):
            state = env.reset()
            rewards = []
            dones = []
            success = False
            for step in range(args.env.horizon):
                if args.env.has_renderer:
                    env.render()
                ...
                if reward>=0.45 and not success:
                    success = True
                    succeed.append(1)
                    break
                state = next_state
            ...
            HORIZON.append(len(rewards))
            print('Ep {}\tHorizon:{} \tMoving average score: {:.2f}\tSuccees{:2f}'.format(epoch, len(rewards), np.array(rewards).sum(),success))
        print("成功率:{}".format(len(succeed)/args.eval.eps))
        SUCCESS_RATE.append(len(succeed)/args.eval.eps)

    SUCCESS_RATE = np.array(SUCCESS_RATE)
    HORIZON = np.array(HORIZON)
    base_name = os.path.basename(os.path.normpath(policy_file))
    success_path = os.path.abspath(os.path.join(policy_file,"../.."))

    if seed_num == 1:
        success_file = f'{success_path}/{len(succeed)/args.eval.eps}_{base_name}.txt'
    else:
        success_file = f'{success_path}/seed_{seed_num}_{SUCCESS_RATE.mean()}.txt'

    with open(success_file, 'w') as f:
        f.write(f'Success_rate_max: {SUCCESS_RATE.max()}\n')
        f.write(f'Success_rate_min: {SUCCESS_RATE.min()}\n')
        f.write(f'Success_rate_mean: {SUCCESS_RATE.mean()}\n')
        f.write(f'Success_rate_std: {SUCCESS_RATE.std()}\n')
        f.write(f'Success_rate: {SUCCESS_RATE}\n')
        f.write(f'Horizon_max: {HORIZON.max()}\n')
        f.write(f'Horizon_min: {HORIZON.min()}\n')
        f.write(f'Horizon_mean: {HORIZON.mean()}\n')
        f.write(f'Horizon_std: {HORIZON.std()}\n')
        f.write(f'Horizon: {HORIZON}\n')
        f.write(f'Model: {args.eval.policy}\n')
