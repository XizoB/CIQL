...
from utils.utils import eval_mode, evaluate, soft_update, save, save_config, save_trajs, select_state
...
def cdynamic(expert_buffer, device):
    """
    ...
    pos = batch_state[:,7:10]
    n_pos = batch_next_state[:,7:10]
    ...
    for i in range(len(v_n)):
        ...
        angle_no = np.degrees(np.arccos(cos_no))
        if batch_reward[i] >= 0.5:
            conf.append(1)
            continue
        if angle_no <= angle_bound:
            conf.append(1)
        else:
            conf.append(0)
...
def main(cfg: DictConfig):
    ...
    if args.C_aware.conf_learn=="IQ" or args.C_aware.conf_learn=="sail":
        ...
        conf = cdynamic(expert_memory_replay, device)

        conf = torch.Tensor(conf)
        print("最优先验概率:", conf.mean())

        if args.C_aware.conf_learn=="max_lamb":
            args.C_aware.lamb = max(conf.mean(), args.C_aware.lamb)
